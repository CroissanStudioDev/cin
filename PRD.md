...
```bash
# Глобальные опции
cin --config <path> <command>         # Использовать проект из указанной директории
cin -c /path/to/project status        # Пример: статус другого проекта
...
cin verify <package> --key <pub>      # Проверить подпись пакета

# Подписание пакетов
cin key generate                      # Генерация Ed25519 ключей для подписи
cin sign <package>                    # Подписать пакет
cin pack --sign                       # Создать и сразу подписать пакет

# Delta transfers (инкрементальные обновления)
cin delta <old.tar.gz> <new.tar.gz>   # Создать delta (только изменения)
cin patch <old.tar.gz> <delta.tar.gz> # Применить delta к старому пакету
...
- [x] `secrets setup/import/list/check/export` — управление секретами
- [x] Шифрование секретов (AES-256-GCM)

### v1.0.0 ✅

- [x] `deploy` — развёртывание с автоматическим backup
- [x] `rollback` — откат к предыдущей версии
- [x] `verify` — проверка пакетов
- [x] `logs` — просмотр логов (live)
- [x] `logs collect` — сбор логов для диагностики
- [x] Sanitizer — автоматическое удаление секретов из логов
- [x] `status` — состояние системы
- [x] Lifecycle хуки: `pre-deploy`, `post-deploy`, `pre-rollback`, `post-rollback`
- [x] `tasks list` / `run <task>` — конфигурируемые задачи
- [x] `--config <path>` — поддержка нескольких проектов от разных студий
- [ ] Полная документация

### v1.0.1 ✅

- [x] Package signing (Ed25519) — `cin key generate`, `cin sign`, `cin verify --key`
- [x] Delta transfers — `cin delta`, `cin patch` (передача только изменений между версиями)
